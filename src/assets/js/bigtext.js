    ...
    BigText = {
      ...
      supports: {
        wholeNumberFontSizeOnly: (function () {
          ...
          var ret =
            computedStyle &&
            computedStyle.getPropertyValue("font-size") === "14px";
          test.remove();
          return ret;
        })(),
      },
        ...
        for (var j = 0, k = linesFontSizes.length; j < k; j++) {
          css.push(
            "#" +
              ...
              (linesFontSizes[j]
                ? " font-size: " + linesFontSizes[j] + "px;"
                : "") +
              (lineWordSpacings[j]
                ? " word-spacing: " + lineWordSpacings[j] + "px;"
                : "") +
              "}"
          );
        }
      ...
      jQueryMethod: function (options) {
        ...
        this.each(function () {
          ...
          $children.addClass(function (lineNumber, className) {
            ...
            return [
              className.replace(
                new RegExp("\\b" + BigText.LINE_CLASS_PREFIX + "\\d+\\b"),
                ""
              ),
              BigText.LINE_CLASS_PREFIX + lineNumber,
            ].join(" ");
          });
          var sizes = BigText.calculateSizes(
            $t,
            $children,
            maxWidth,
            options.maxfontsize,
        ...
        if (width >= maxWidth) {
          ...
          return {
            ...
            size: parseFloat(
              (
                parseFloat(size) -
                (property === "word-spacing" && previousWidth && over < under
                  ? 0
                  : interval)
              ).toFixed(3)
            ),
        ...
        $children
          ...
          .each(function () {
            ...
            outer: for (var m = 0, n = intervals.length; m < n; m++) {
              inner: for (var j = 1, k = 10; j <= k; j++) {
                ...
                lineMax = BigText.testLineDimensions(
                  $line,
                  maxWidth,
                  "font-size",
          ...
          .each(function (lineNumber) {
            ...
            for (var m = 1, n = 3; m < n; m += interval) {
              maxWordSpacing = BigText.testLineDimensions(
                $line,
                maxWidth,
                "word-spacing",
                m,
